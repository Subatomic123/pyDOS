...
while True:
	...
	if user.lower() == "help":
		print("commands are: help, sysinfo, sysver, whoami, filesys, unroot, root, del, state, A:, B: C:, D:, shutdown, ls /directory/ (must be on /user/), {program name}, fileinfo {file}, programs (to list programs)")
		
	if user.lower() == "programs":
		print("Apps are: calc.exe, coming_soon.exe")
