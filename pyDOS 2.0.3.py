...
def calc():
    if state["cannot_open_programs"]:
        print("opening...")
        time.sleep(1.3)
        print("CRITICAL: SYS.SYS = NULL")
    elif state["calc_deleted"]:
    	print("cannot find 'calc.exe'")
    else:
        print("|=================|")
        print("|(         Calculator           )|")
        print("|=================|")
...
def money():
		money = 100
		if state["money_sim_del"]:
			print("cannot find 'money_sim.exe'")
		elif state["cannot_open_programs"]:
			print("CRITICAL: SYS.SYS = NULL")
		else:
			import random
			print("|===============|")
			print("|        money sim        |")
			print("|===============|")
			money1 = int(input("How much money?"))
			if money1 >= money:
...
while True:
	...
	if user.lower() == "help":
		print("commands are: help, sysinfo, sysver, whoami, filesys, unroot, root, del, state, A:, B: C:, D:, shutdown, ls /user/, program name")
	...
	if user.lower() == "sysinfo":
		...
		if state["Disk_C"]:
		    print("C:/")
		elif state["system_and_bin_deleted"]:
		    	print("/")
		    	print("/daemon/")
	...
	elif user.lower() == "del calc.exe":
		print("deleted")
		state["calc_deleted"] = True
		
		
	elif user.lower() == "money_sim.exe":
		print("opening...")
		time.sleep(2.6)
		money()
