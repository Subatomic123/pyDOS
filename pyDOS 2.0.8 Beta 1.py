...
def safemode():
	...
	print("type 'help' for help")
	print("type 'unmode' to return to normal system")
	print("type 'bios' to go to bios")
	while True:
		safeuser = input("RAM:/")
		if state["dummy"]:
			print("")
		elif safeuser.lower() == "bios":
			time.sleep(1)
			print("going to bios...")
			time.sleep(3)
			bios()
...
while True:
	...
	if user.lower() == "help":
		print("commands are: help, sysinfo, sysver, whoami, filesys, unroot, root, del, state, A:, B: C:, D:, shutdown, ls /directory/ (must be on /user/), {program name}.exe, fileinfo {file}, programs (to list programs), numprog (to show number of programs)")
