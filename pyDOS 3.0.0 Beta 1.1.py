...
filesys = (["/system/", "/bin/", "/init/, /dalvik/", "init.rc", "/daemon/"])
...
def devtool():
    ...
    if user.lower() == "help":
        print("bad behavior: hardbrick, bootloop, crash, kernel panic")
    if user.lower() == "hardbrick":
        hardbrick()
    if user.lower() == "bootloop":
        bootloop()
    if user.lower() == "crash":
        state["crash"] = True
        DOS()
...
def post():
            if "/system/" not in filesys and "/bin/" not in filesys:
                hardbrick()
            else:
                state["booting"] = True
